...
export default function WordImport({ langCode, owner }: { langCode: string, owner: string }) {
            ...
            {mode === "export" && (
                <div>
                    ...
                    <textarea
                        ...
                        readOnly
                    />
                    {importResult && <div className="text-red-500 mt-2">{importResult}</div>}
                </div>
            )}
            {mode === "import" && (
                <form onSubmit={handleImport}>
                    <label className="block mb-2 font-medium">Paste the CSV to import:</label>
                    <textarea
                        className="w-full h-48 p-2 border rounded text-xs"
                        value={csv}
                        onChange={e => setCsv(e.target.value)}
                        placeholder={CSV_FIELDS.join(",") + "\nlexical_item,definition,pos,notes,transliteration,..."}
                        required
                    />
                    <Button type="submit" className="mt-2" disabled={loading}>{loading ? "Importing..." : "Import"}</Button>
                    {importResult && <div className={importResult.startsWith("Import completed") ? "text-green-600 mt-2" : "text-red-500 mt-2"}>{importResult}</div>}
                </form>
            )}
        </div>
    );
