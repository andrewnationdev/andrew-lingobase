...
export default function UserPageComponent({
  ...
  const [isReadyToDisplayUser, setIsReadyToDisplayUser] = useState(false);
    ...
    try {
      ...
      if (data && data.username) {
        if (
          ...
          (userAlias.length < 8 ||
            userAlias.length > 16 ||
            !/^[A-Za-z0-9_]+$/.test(userAlias))
        ) {
          showErrorToast(
            "FATAL ERROR: Username must be 8-16 characters and can only contain letters, numbers, and underscores.",
          );
          setIsLoading(false);
          return;
        }
  ...
  return (
    <div className="flex-1 w-full flex flex-col gap-8">
      ...
      {!isReadyToDisplayUser && <LoadingComponent />}
      {/* User Description Section */}
      {isReadyToDisplayUser && (
        <>
            ...
            {isEditing ? (
              <div className="space-y-4">
                <div className="flex flex-col gap-4">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    - They must have between 8 and 16 letters
                  </p>
                </div>
                <textarea
          ...
          <span className="text-center mb-2">
            This user has <span className="font-bold">{conlangCount}</span>{" "}
            conlangs on the website!
          </span>
