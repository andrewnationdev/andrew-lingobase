...
export default function ArticleView({
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      const conlangResponse = await supabase
        .from("conlang")
        .select("english_name")
        .eq("code", conlangCode)
        .single();
                ...
                <div className="mt-4 space-y-2">
                  ...
                  {loggedUser == article.written_by && (
                    <>
                      ...
                      <button
                        className="ml-4 px-4 py-2 bg-red-500 text-white font-medium text-sm rounded-full shadow-md hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition-colors duration-200"
