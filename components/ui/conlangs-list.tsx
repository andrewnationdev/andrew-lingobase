...
export default function ConlangsList({ authOnly = false, user }: { authOnly?: boolean, user?: string }) {
  ...
  return <div className="flex flex-col w-full space-y-4 font-sans">
    {conlangs.length > 0 && conlangs.map(conlang => (
    ...
    {conlangs.length == 0 && <div className="bg-accent text-sm p-3 px-5 rounded-md text-foreground flex gap-3 items-center">
